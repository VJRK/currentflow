...
taste_hoch = pygame.transform.rotate(pygame.transform.scale(pygame.image.load('tasten/hoch.png'), (int(gv.width / 40), int(gv.width / 40))), 90)
...
def render(canvas):
    ...
    for j in range(len(stage.all_levels)):
        if j % 3 == 0:
            canvas.blit(text_surfaces_lev[j],
                    (gv.width * 1 / 6 - rects_lev[j][2] / 2, gv.height / 4 + col * gv.width / 20 - rects_lev[j][3] / 2))
        if j % 3 == 1:
            canvas.blit(text_surfaces_lev[j],
                    (gv.width * 3 / 6 - rects_lev[j][2] / 2, gv.height / 4 + col * gv.width / 20 - rects_lev[j][3] / 2))
        if j % 3 == 2:
            canvas.blit(text_surfaces_lev[j],
                    (gv.width * 5 / 6 - rects_lev[j][2] / 2, gv.height / 4 + col * gv.width / 20 - rects_lev[j][3] / 2))
            col += 1
    # Exit Menu
    ...
    # Taste hoch
    canvas.blit(taste_hoch, (gv.width * 1 / 50, gv.height * 17 / 20))
    # Taste w
    canvas.blit(taste_w, (gv.width * 3 / 50, gv.height * 17 / 20))
    # Taste runter
    canvas.blit(taste_runter, (gv.width * 1 / 50, gv.height * 18 / 20))
    # Taste s
